...
class F1Metric(BaseMetric):
    ...
    def __init__(self,
                 ...
                 iou_thrs: Optional[Union[float, Sequence[float]]] = [0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95],
    ...
    def compute_metrics(self, results: list) -> dict:
        ...
        if 'mAP' in self.metrics:
            ...
            for iou_thr in self.iou_thrs:
                ...
                eval_results[f'MaxF1Score{int(iou_thr * 100):02d}'] = round(float(max_f1_score), 5)
                eval_results[f'MaxRecallF{int(iou_thr * 100):02d}'] = round(float(recall_in_max_f1_score), 5)
                eval_results[f'MaxprecisionF{int(iou_thr * 100):02d}'] = round(float(precision_in_max_f1_score), 5)

                eval_results[f'AP{int(iou_thr * 100):02d}'] = round(float(mean_ap), 3)
            eval_results['mAP'] = float(sum(mean_aps) / len(mean_aps))
